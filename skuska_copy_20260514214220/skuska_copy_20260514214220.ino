...
// ----- отрисовка
#define START_MODE 0                    // режим при запуске
...
#define MID_COLOR HUE_AQUA       // цвет средних
...
boolean webMonitoring = false;
boolean webOpened = false;

unsigned long serialSendTimer = 0;
#define SERIAL_SEND_PERIOD 200
...
void loop() {
  serialTick();     // команды от Flask / Raspberry Pi
  ...
  mainLoop();
  serialTelemetry();  // отправка данных в Web
  eepromTick();
}
...
void serialTick() {
  if (Serial.available()) {
    ...
    if (cmd == "OPEN") {
      ...
      String modeValue = cmd.substring(5);

      if (modeValue == "Sound monitor") {
        this_mode = 0;
      }
      else if (modeValue == "LED visualizer") {
        this_mode = 2;
      }
      else if (modeValue == "Calibration") {
        fullLowPass();
      }
      else {
        int modeNumber = modeValue.toInt();

        if (modeNumber >= 0 && modeNumber < MODE_AMOUNT) {
          this_mode = modeNumber;
        }
      }
    }
  }
...
void serialTelemetry() {
  if (!webOpened) return;
  if (!webMonitoring) return;

  if (millis() - serialSendTimer >= SERIAL_SEND_PERIOD) {
    serialSendTimer = millis();

    int rawSound = analogRead(SOUND_R);
    int freqSound = analogRead(SOUND_R_FREQ);

    Serial.print("{");
    Serial.print("\"sound\":");
    Serial.print(rawSound);

    Serial.print(",\"freq\":");
    Serial.print(freqSound);

    Serial.print(",\"mode\":");
    Serial.print(this_mode);

    Serial.print(",\"brightness\":");
    Serial.print(BRIGHTNESS);

    Serial.print(",\"on\":");
    Serial.print(ONstate ? 1 : 0);
